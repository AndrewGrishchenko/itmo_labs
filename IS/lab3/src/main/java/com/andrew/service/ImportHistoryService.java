...
public class ImportHistoryService {
    ...
    public ImportHistory createImportHistory(User user, OperationStatus operationStatus, Integer objectCount) {
    ...
    @Transactional(Transactional.TxType.REQUIRES_NEW)
    public void update(ImportHistory importHistory) {
        importHistoryRepository.update(importHistory);
    }

    @Transactional(Transactional.TxType.REQUIRES_NEW)
    public void markError(ImportHistory importHistory) {
        importHistoryRepository.markError(importHistory);
    }

    @Transactional
    public void markOk(ImportHistory importHistory) {
        importHistoryRepository.markOk(importHistory);
    }
    ...
    public ImportHistoryResponse getById(int id) {
        ...
        if (importHistory.getUser().getId() != currentUser.getUser().getId() && !currentUser.isAdmin())
