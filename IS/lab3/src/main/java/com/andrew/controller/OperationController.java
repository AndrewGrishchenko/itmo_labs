...
import com.andrew.infra.L2CacheStatsInterceptor;
...
public class OperationController {
    ...
    public Response downloadImportFile(@PathParam("id") int id) {
        InputStream fileStream = operationService.downloadFile(id);

        return Response.ok(fileStream)
            .header("Content-Disposition", "attachment; filename=\"import_" + id + ".json\"")
            .build();
    }
    ...
    @POST
    @Path("/cache/logging")
    public void setCacheLogging(@QueryParam("enabled") boolean enabled) {
        L2CacheStatsInterceptor.setEnabled(enabled);
    }

    @POST
    @Path("enable")
    public Response enableFailure() {
        OperationService.setFailure(true);
        return Response.ok().build();
    }

    @POST
    @Path("disable")
    public Response disableFailure() {
        OperationService.setFailure(false);
        return Response.ok().build();
    }
}
