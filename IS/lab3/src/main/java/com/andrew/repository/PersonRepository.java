...
public class PersonRepository {
    ...
    public List<Person> getAllUser(User user) {
        return sessionFactory.getCurrentSession().createQuery("from Person p where p.owner = :user", Person.class)
    ...
    private PageResponse<Person> findInternal(User user, int page, int size, String sortField, String sortOrder, PersonFilter filter) {
        ...
        Query<Person> query = sessionFactory.getCurrentSession().createQuery(hql, Person.class);
...
public Optional<Person> findById(Long id) {
        return sessionFactory.getCurrentSession().createQuery("from Person p where p.id = :id", Person.class)
                      .setParameter("id", id)
                      .uniqueResultOptional();
    }

    public List<Person> getAllWithLocation(Location location) {
        return sessionFactory.getCurrentSession().createQuery("from Person p where p.location = :location", Person.class)
                      .setParameter("location", location)
                      .list();
    }

    public boolean existsByLocationAndNotOwner(Location location, User owner) {
        return sessionFactory.getCurrentSession().createQuery("select count(p.id) from Person p where " +
    ...
    public Person update(Person person) {
        Person updated = sessionFactory.getCurrentSession().merge(person);
        return updated;
    }
